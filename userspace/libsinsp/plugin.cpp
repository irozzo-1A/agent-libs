...
static void plugin_log_fn(ss_plugin_owner_t* o,
...
bool sinsp_plugin::init(const std::string& config, std::string& errstr) {
	...
	if(m_caps & CAP_EXTRACTION) {
		/* Here we populate the `m_extract_event_codes` for the plugin, while
		 * `m_extract_event_sources` is already populated in the plugin_init */
	...
	if(m_caps & CAP_PARSING) {
		/* Here we populate the `m_parse_event_codes` for the plugin, while `m_parse_event_sources`
		 * is already populated in the plugin_init */
...
void sinsp_plugin::resolve_dylib_field_arg(Json::Value root, filtercheck_field_info& tf) {
	...
	if(!isRequired.isNull()) {
		if(!isRequired.isBool()) {
			throw sinsp_exception(string("error in plugin ") + m_name + ": field " + tf.m_name +
			                      " isRequired property is not boolean");
		}
		...
		if(isRequired.asBool() == true) {
			...
			tf.m_flags = (filtercheck_field_flags)((int)tf.m_flags |
			                                       (int)filtercheck_field_flags::EPF_ARG_REQUIRED);
		}
	}
	...
	if(!isIndex.isNull()) {
		...
		if(isIndex.asBool() == true) {
			...
			tf.m_flags = (filtercheck_field_flags)((int)tf.m_flags |
			                                       (int)filtercheck_field_flags::EPF_ARG_INDEX);
			tf.m_flags = (filtercheck_field_flags)((int)tf.m_flags |
			                                       (int)filtercheck_field_flags::EPF_ARG_ALLOWED);
		}
	}
	...
	if(!isKey.isNull()) {
		if(!isKey.isBool()) {
			throw sinsp_exception(string("error in plugin ") + m_name + ": field " + tf.m_name +
			                      " isKey property is not boolean");
		}

		if(isKey.asBool() == true) {
			// We set `EPF_ARG_ALLOWED` implicitly.
			tf.m_flags = (filtercheck_field_flags)((int)tf.m_flags |
			                                       (int)filtercheck_field_flags::EPF_ARG_KEY);
			tf.m_flags = (filtercheck_field_flags)((int)tf.m_flags |
			                                       (int)filtercheck_field_flags::EPF_ARG_ALLOWED);
		}
	}
	...
	if((tf.m_flags & filtercheck_field_flags::EPF_ARG_REQUIRED) &&
	   !(tf.m_flags & filtercheck_field_flags::EPF_ARG_INDEX ||
		...
		throw sinsp_exception(string("error in plugin ") + m_name + ": field " + tf.m_name +
		                      " arg has isRequired true, but none of isKey nor isIndex is true");
	}
	return;
}

// this logic is shared between the field extraction and event parsing caps
void sinsp_plugin::resolve_dylib_compatible_codes(uint16_t* (*get_codes)(uint32_t*, ss_plugin_t*),
...
static void resolve_dylib_json_strlist(const std::string& plname,
	...
	if(jsonstr.empty()) {
		if(allow_empty) {
			...
			throw sinsp_exception("error in plugin " + plname + ": '" + symbol +
			                      "' did not return a json array but it should");
		}
	}
...
// this logic is shared between the field extraction and event parsing caps
void sinsp_plugin::resolve_dylib_compatible_sources(const std::string& symbol,
...
bool sinsp_plugin::resolve_dylib_symbols(std::string& errstr) {
	...
	std::string req_api_version_str =
	        str_from_alloc_charbuf(m_handle->api.get_required_api_version());
	...
	if(m_caps & CAP_SOURCING) {
		...
		if(m_handle->api.get_id != NULL && m_handle->api.get_event_source != NULL &&
		   m_handle->api.get_id() != 0) {
			...
			if(m_event_source == sinsp_syscall_event_source_name) {
				errstr =
				        "plugin can't implement the reserved event source '" + m_event_source + "'";
				return false;
			}
		}
	}
	...
	if(m_caps & CAP_EXTRACTION) {
		...
		if(sfields == NULL) {
			throw sinsp_exception(string("error in plugin ") + name() +
			                      ": get_fields returned a null string");
		}
		string json(sfields);
		...
		if(Json::Reader().parse(json, root) == false || root.type() != Json::arrayValue) {
			throw sinsp_exception(string("error in plugin ") + name() +
			                      ": get_fields returned an invalid JSON");
		}
		...
		for(Json::Value::ArrayIndex j = 0; j < root.size(); j++) {
			...
			if(!jvIsList.isNull()) {
				...
				if(jvIsList.asBool()) {
					tf.m_flags =
					        (filtercheck_field_flags)((int)tf.m_flags |
			...
			if(!jvProperties.isNull()) {
				...
				for(const auto& prop : jvProperties) {
					if(!prop.isString()) {
						throw sinsp_exception(string("error in plugin ") + name() + ": field " +
						                      fname + " properties value is not string ");
					}

					const std::string& str = prop.asString();

					// "hidden" is used inside and outside libs. "info" and "conversation" are used
					// outside libs.
					if(str == "hidden") {
						tf.m_flags = (filtercheck_field_flags)((int)tf.m_flags |
						                                       (int)filtercheck_field_flags::
	...
	if(m_caps & CAP_ASYNC) {
		resolve_dylib_compatible_sources("get_async_event_sources",
		                                 m_handle->api.get_async_event_sources,
		                                 m_async_event_sources);
		resolve_dylib_json_strlist(name(),
		                           "get_async_events",
	...
	if(!validator.validate(schemaDef, configAdapter, &validationResults)) {
		...
		if(validationResults.popError(error)) {
			throw sinsp_exception(
			        string("error in plugin ") + name() + " init config: In " +
...
static void set_plugin_metric_value(metrics_v2& metric,
                                    metrics_v2_value_type type,
                                    ss_plugin_metric_value val) {
	switch(type) {
...
ss_plugin_routine_t* plugin_subscribe_routine(ss_plugin_owner_t* o,
                                              ss_plugin_routine_fn_t r,
                                              ss_plugin_routine_state_t* s) {
	auto t = static_cast<sinsp_plugin*>(o);
...
scap_source_plugin& sinsp_plugin::as_scap_source() {
	...
	if(!(caps() & CAP_SOURCING)) {
		throw sinsp_exception(
		        "can't create scap_source_plugin from a plugin without CAP_SOURCING capability.");
	}
...
std::string sinsp_plugin::get_progress(uint32_t& progress_pct) const {
	...
	uint32_t ppct;
	ret = str_from_alloc_charbuf(
	        m_handle->api.get_progress(m_state, m_scap_source_plugin.handle, &ppct));
...
std::vector<sinsp_plugin::open_param> sinsp_plugin::list_open_params() const {
	...
	if(m_state && m_handle->api.list_open_params) {
		...
		if(rc != SS_PLUGIN_SUCCESS) {
			throw sinsp_exception(string("error in plugin ") + name() +
			                      ": list_open_params has error " + get_last_error());
		}
		...
		if(jsonString.size() > 0) {
			...
			if(Json::Reader().parse(jsonString, root) == false || root.type() != Json::arrayValue) {
				throw sinsp_exception(string("error in plugin ") + name() +
				                      ": list_open_params returned a non-array JSON");
			}
			for(Json::Value::ArrayIndex i = 0; i < root.size(); i++) {
				...
				if(param.value == "") {
					throw sinsp_exception(string("error in plugin ") + name() +
					                      ": list_open_params has entry with no value");
				}
...
std::unique_ptr<sinsp_filter_check> sinsp_plugin::new_filtercheck(
...
ss_plugin_rc sinsp_plugin::handle_plugin_async_event(ss_plugin_owner_t* o,
                                                     const ss_plugin_event* e,
                                                     char* err) {
	...
	if(!(p->caps() & CAP_ASYNC)) {
		if(err) {
			strlcpy(err,
			        "plugin without async events cap used as async handler",
			        PLUGIN_MAX_ERRLEN);
		}
