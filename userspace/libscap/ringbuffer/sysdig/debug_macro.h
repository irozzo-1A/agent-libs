...
#define DEBUG_ACTIVE_LIST()                                                            \
	...
	for(uint16_t i = devset->al.h; i != INVALID_BUFFER_ID; i = devset->buf_array[i]) { \
		if(devset->buf_array[i] == INVALID_BUFFER_ID) {                                \
			DEBUG_MSG("%d", i);                                                        \
		} else {                                                                       \
			DEBUG_MSG("%d->", i);                                                      \
		}                                                                              \
	}                                                                                  \
	DEBUG_MSG(" (head: %d,tail: %d)\n", devset->al.h, devset->al.t);

#define DEBUG_WAIT_LIST()                                                                 \
	DEBUG_MSG("Wait list: ");                                                             \
	for(uint16_t i = devset->wl_head; i != INVALID_BUFFER_ID; i = devset->buf_array[i]) { \
		if(devset->buf_array[i] == INVALID_BUFFER_ID) {                                   \
			DEBUG_MSG("%d", i);                                                           \
		} else {                                                                          \
			DEBUG_MSG("%d->", i);                                                         \
