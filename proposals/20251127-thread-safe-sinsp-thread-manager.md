...
The current `sinsp_thread_manager` operates in a single-threaded environment, utilizing a `std::unordered_map` plus some auxiliary data structures to represent the threads topology without any synchronization. This offers fast lookups but strictly limits Falco's event processing throughput to the speed of a single CPU core.
...
## Proposed Architecture: Atomic pointers array
...
This allows N parsers to read thread state simultaneously without communicating with each other or the writer.
...
struct sinsp_thread_info {
    ...
    std::atomic<sinsp_thread_info*> m_ptid{nullptr}; // parent thread pointer
    list_node<sinsp_thread_info> m_children; // intrusive list of children

    // Intrusive list node for fast thread group leader traversal
    list_head m_tasks{nullptr};
...
private:
    ...
    std::atomic<sinsp_thread_info*> list_head_{nullptr};
...
The navigation of the linked structure is something common both on the read and the read side e.g.
...
void for_each_thread(thread_visitor callback) {
    rcu_read_lock(); // Begin RCU critical section
    sinsp_thread_info* current = rcu_dereference(list_head_);
    while (current) {
        // Execute the user's logic *inside* the RCU critical section
        callback(current);
...
Storing the threads sequentially may lead to false sharing issues, in case we do we may want to use a mapping function for indexes to avoid inserting pointers in contiguous memory.

## Bibliography

* [https://lwn.net/Articles/262464/](https://lwn.net/Articles/262464/) What is RCU, Fundamentally?
* [https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2023/p2530r2.pdf](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2023/p2530r2.pdf)
* [https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2022/p2545r1.pdf](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2022/p2545r1.pdf)
