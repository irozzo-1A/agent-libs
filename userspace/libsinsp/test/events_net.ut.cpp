...
TEST_F(sinsp_with_test_input, net_socket) {
	...
	ASSERT_EQ(get_field_as_string(evt, "fd.l4proto"),
	          "<NA>");  /// todo: probably this is not what we want
...
TEST_F(sinsp_with_test_input, net_ipv4_connect) {
	...
	ASSERT_EQ(tinfo->m_latency.load(), 0);
	...
	ASSERT_TRUE(fdinfo->is_ipv4_socket());
	ASSERT_TRUE(
	        fdinfo->is_socket_connected()); /* in the parse exit we set the socket as connected */
	...
	/* The concept of remote ip is quite strange, we check if the client address is one of our
	 * interfaces, if yes the remote ip will be the server otherwise it will be the client! In this
	 * case, the client IP is completely random so it will be considered as remote, while the server
	 * ip will be local!
	 */
...
TEST_F(sinsp_with_test_input, net_ipv6_multiple_connects) {
	...
	std::vector<uint8_t> server2_sockaddr =
	        test_utils::pack_sockaddr(reinterpret_cast<sockaddr*>(&server2));
	...
	/* check that upon entry to the new connect the fd name is the same as during the last
	 * connection */
	ASSERT_EQ(get_field_as_string(evt, "fd.name"), DEFAULT_IPV6_FDNAME);
...
TEST_F(sinsp_with_test_input, net_bind_listen_accept_ipv4) {
	...
	std::string fdname =
	        std::string(DEFAULT_IPV4_SERVER_STRING) + ":" + std::string(DEFAULT_SERVER_PORT_STRING);
	ASSERT_EQ(get_field_as_string(evt, "fd.name"), fdname);
	ASSERT_EQ(get_field_as_string(evt, "fd.is_server"), "true");
	ASSERT_EQ(get_field_as_string(evt, "fd.sip"), DEFAULT_IPV4_SERVER_STRING);
	ASSERT_FALSE(field_has_value(evt, "fd.cip")); /* we are not able to retrieve the client ip, the
	                                                 fdinfo type is SCAP_FD_IPV4_SERVSOCK */
	ASSERT_FALSE(field_has_value(evt, "fd.rip")); /* we are not able to retrieve remote ip, the
	                                                 fdinfo type is SCAP_FD_IPV4_SERVSOCK */
	ASSERT_FALSE(field_has_value(evt, "fd.lip")); /* we are not able to retrieve local ip, the
	                                                 fdinfo type is SCAP_FD_IPV4_SERVSOCK */
	ASSERT_EQ(get_field_as_string(evt, "fd.sport"), DEFAULT_SERVER_PORT_STRING);
	ASSERT_FALSE(field_has_value(evt, "fd.cport"));
	ASSERT_FALSE(field_has_value(evt, "fd.rport"));
	...
	std::vector<uint8_t> st = test_utils::pack_socktuple(reinterpret_cast<sockaddr*>(&client),
	                                                     reinterpret_cast<sockaddr*>(&server));
	...
	add_event_advance_ts(increasing_ts(),
	                     ...
	                     5,
	                     new_connected_fd,
	                     scap_const_sized_buffer{st.data(), st.size()},
	                     (uint8_t)0,
	                     (uint32_t)0,
	                     (uint32_t)5);
...
TEST_F(sinsp_with_test_input, net_connect_enter_event_is_empty) {
	...
	socktuple = test_utils::pack_socktuple(reinterpret_cast<sockaddr*>(&client2),
	                                       reinterpret_cast<sockaddr*>(&server2));
	evt = add_event_advance_ts(increasing_ts(),
	                           1,
	                           PPME_SOCKET_CONNECT_X,
...
TEST_F(sinsp_with_test_input, net_connect_enter_event_is_missing) {
	...
	std::string fdname =
	        ipv4_client + ":" + port_client_string + "->" + ipv4_server + ":" + port_server_string;
	ASSERT_EQ(get_field_as_string(evt, "fd.name"), fdname);
