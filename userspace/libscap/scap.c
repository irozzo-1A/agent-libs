...
int32_t scap_init(scap_t* handle, scap_open_args* oargs, const struct scap_vtable* vtable) {
	...
	//
	// The kmod hooks in the scap_linux_vtable need an initialized engine, since they call ioctls on
	// the driver fd, so we need to initialize the engine before the platform.

	return scap_init_engine(handle, oargs, vtable);
}
