...
TEST(sinsp_threadinfo, get_main_thread) {
	...
	ASSERT_EQ(tinfo->get_main_thread(), main_tinfo);
}
...
TEST(sinsp_threadinfo, get_num_threads) {
	...
	/* Please note that here we still have 2 because we have just marked the thread as Dead without
	 * decrementing the alive count */
	ASSERT_EQ(tinfo->get_num_threads(), 2);
