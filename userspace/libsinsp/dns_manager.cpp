...
using concurrent_map = tbb::concurrent_unordered_map<A, B>;
template<typename A, typename B, typename C>
using concurrent_map_hash = tbb::concurrent_unordered_map<A, B, C>;
#else
#include <unordered_map>
template<typename A, typename B>
using concurrent_map = std::unordered_map<A, B>;
template<typename A, typename B, typename C>
using concurrent_map_hash = std::unordered_map<A, B, C>;
#endif

#define G_LOG_FORMAT(s, fmt, ...)                                                 \
	do {                                                                          \
		if(s >= libsinsp_logger()->get_severity()) {                              \
			libsinsp_logger()->format(sinsp_logger::SEV_DEBUG, fmt, __VA_ARGS__); \
		}                                                                         \
	} while(0)
...
struct af_hash_func<AF_INET> {
	...
	std::size_t operator()(const type& v) const { return std::hash<type>{}(v); }
};
...
struct void_ptr_to_address<AF_INET> {
	af_type_traits<AF_INET>::type operator()(const void* addr) const { return *(uint32_t*)addr; }
};
...
public:
	...
	// insert from resolve
	bool insert(addrinfo* addr, uint64_t ts) { return insert(to_address<AF>(addr), ts); }
...
public:
		...
		*m_v6_addrs = *other.m_v6_addrs;
		...
		if(ts > m_last_resolve_ts + refresh_timeout) {
			...
			G_LOG_FORMAT(
			        ...
			        "dns_info resolve: name: %s, updated: %s, next refresh in: %llu sec, ipv4: "
			        "[%s], "
			        "ipv6: [%s]",
			        name.c_str(),
			        updated ? "true" : "false",
	...
	bool has_address(int af, void* addr, uint64_t ts) const {
		auto ret = (af == AF_INET) ? m_v4_addrs->has_address(addr, ts)
		                           : m_v6_addrs->has_address(addr, ts);
	...
	void copy_addrs(const addr_consumer_t<AF_INET>& consumer) const { m_v4_addrs->copy(consumer); }

	void copy_addrs(const addr_consumer_t<AF_INET6>& consumer) const { m_v6_addrs->copy(consumer); }
...
public:
	...
	void insert(const std::string& name, const dns_info* info) {
		info->copy_addrs([&](const addr_t& addr) { m_map[addr] = std::make_pair(name, info); });
	}
...
private:
	concurrent_map_hash<addr_t, std::pair<std::string, const dns_info*>, af_hash_func<AF>> m_map;
};

// combined v4, v6 maps
class dns_af_cache {
public:
	enum match_result { MATCH_OK, MATCH_NO_ADDRESS, MATCH_NO_NAME };
	...
	std::size_t size() const { return m_info_table.size(); }

	// async
	bool refresh(std::shared_ptr<dns_af_cache> shadow_cache,
	             uint64_t erase_timeout,
...
private:
	concurrent_map<std::string, std::shared_ptr<const dns_info>> m_info_table;
...
bool sinsp_dns_manager::match(const char* name, int af, void* addr, uint64_t ts) {
	...
	m_dns_cache->insert(name,
	                    std::make_shared<dns_info>(name,
	                                               m.m_base_refresh_timeout,
	                                               m.m_max_refresh_timeout,
	                                               ts));
