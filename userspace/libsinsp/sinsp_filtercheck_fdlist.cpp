	...
	for(j = 0; j < nfds; j++) {
		...
		case TYPE_CLIENTIPS: {
			if(fdinfo != NULL) {
				...
				if(fdinfo->m_type == SCAP_FD_IPV4_SOCK) {
					inet_ntop(AF_INET,
					          &fdinfo->m_sockinfo.m_ipv4info.m_fields.m_sip,
					          m_addrbuff,
					          sizeof(m_addrbuff));
					m_strval += m_addrbuff;
					break;
		...
		case TYPE_SERVERIPS: {
			if(fdinfo != NULL) {
				...
				if(fdinfo->m_type == SCAP_FD_IPV4_SOCK) {
					inet_ntop(AF_INET,
					          &fdinfo->m_sockinfo.m_ipv4info.m_fields.m_dip,
					          m_addrbuff,
					          sizeof(m_addrbuff));
					m_strval += m_addrbuff;
					break;
					...
					inet_ntop(AF_INET,
					          &fdinfo->m_sockinfo.m_ipv4serverinfo.m_ip,
					          m_addrbuff,
					          sizeof(m_addrbuff));
					m_strval += m_addrbuff;
					break;
					...
					inet_ntop(AF_INET,
					          &fdinfo->m_sockinfo.m_ipv6serverinfo.m_ip.m_b,
					          m_addrbuff,
					          sizeof(m_addrbuff));
					m_strval += m_addrbuff;
					break;
