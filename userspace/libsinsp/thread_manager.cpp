...
typename threadinfo_map_impl_t<SyncPolicy>::ptr_t sinsp_thread_manager_impl<SyncPolicy>::get_thread(
	...
	if(!sinsp_proc && (m_threadtable.size() < m_max_thread_table_size || tid == m_sinsp_pid)) {
		...
		// Check if the rate-limit period has elapsed and reset counters.
		// This must happen outside the rate-limited block; otherwise,
		// once the limit is hit, the counters never reset and all
		// subsequent /proc lookups are permanently blocked.
		if(m_proc_lookup_period) {
			const uint64_t now = sinsp_utils::get_current_time_ns();
			const uint64_t elapsed = now - m_last_proc_lookup_period_start.load();
			if(elapsed >= m_proc_lookup_period) {
				reset_thread_counters();
				m_last_proc_lookup_period_start.store(now);
			}
		}
		...
		if(thread_fetched) {
			m_n_get_thread_proc_adds.fetch_add(1, std::memory_order_relaxed);
		}

		if(!thread_fetched) {
			m_n_get_thread_fake_adds.fetch_add(1, std::memory_order_relaxed);
			if(!create_fake) {
				return {};
			}
...
bool sinsp_thread_manager_impl<SyncPolicy>::has_recently_exited(int64_t tid,
	 ...
	 *
	 * The comparison must work in both directions: in multi-buffer mode
	 * a parent's clone_exit (earlier timestamp) can be processed after
	 * the child's PROC_EXIT (later timestamp) was already handled by
	 * another worker.
	 */
	...
	for(size_t i = 0; i < RECENTLY_EXITED_RING_SIZE; i++) {
		if(m_recently_exited_tids[i].key == key) {
			const uint64_t diff = ts >= m_recently_exited_tids[i].ts
			                              ? (ts - m_recently_exited_tids[i].ts)
			                              : (m_recently_exited_tids[i].ts - ts);
			if(diff < MAX_AGE_NS) {
				libsinsp_logger()->format(sinsp_logger::SEV_INFO,
				                          "recently_exited: suppressed re-add of tid %" PRId64
				                          " (ptid %" PRId64 ", diff_ns %" PRIu64
				                          ", query_ts %" PRIu64 ", exit_ts %" PRIu64 ")",
				                          tid,
				                          ptid,
				                          diff,
				                          ts,
				                          m_recently_exited_tids[i].ts);
				return true;
			}
		}
	}
...
bool sinsp_thread_manager_impl<SyncPolicy>::remove_inactive_threads() {
	...
	uint64_t expected = m_last_flush_time_ns.load(std::memory_order_relaxed);
	if(expected == 0) {
		...
		if(m_threads_purging_scan_time_ns > 30 * ONE_SECOND_IN_NS) {
			init_val = last_event_ts - m_threads_purging_scan_time_ns + 30 * ONE_SECOND_IN_NS;
		} else {
			init_val = last_event_ts - m_threads_purging_scan_time_ns;
		}
		m_last_flush_time_ns.compare_exchange_strong(expected, init_val, std::memory_order_acq_rel);
		expected = m_last_flush_time_ns.load(std::memory_order_relaxed);
	}

	if(last_event_ts <= expected + m_threads_purging_scan_time_ns) {
		return false;
	}

	// Use CAS so that exactly one thread wins when multiple workers call concurrently.
	if(!m_last_flush_time_ns.compare_exchange_strong(expected,
	                                                 last_event_ts,
	                                                 std::memory_order_acq_rel)) {
		return false;
	}

	const auto table_size_before = m_threadtable.size();
	libsinsp_logger()->format(sinsp_logger::SEV_INFO,
	                          "Purging thread table: size_before=%zu, max=%u, "
	                          "proc_adds=%" PRIu64 ", fake_adds=%" PRIu64,
	                          table_size_before,
	                          m_max_thread_table_size,
	                          m_n_get_thread_proc_adds.load(std::memory_order_relaxed),
	                          m_n_get_thread_fake_adds.load(std::memory_order_relaxed));
	...
	size_t n_dead = 0;
	size_t n_alive = 0;
	loop_threads([&to_delete, &n_invalid, &n_stale, &n_dead, &n_alive, last_event_ts, this](
	                     const sinsp_threadinfo_impl<SyncPolicy>& tinfo) {
		if(tinfo.is_dead()) {
			n_dead++;
		} else {
			n_alive++;
		...
		if(tinfo.is_invalid()) {
			...
			n_invalid++;
		} else if(last_event_ts > tinfo.get_lastaccess_ts() + m_thread_timeout_ns &&
	...
	const auto table_size_after = m_threadtable.size();
	libsinsp_logger()->format(sinsp_logger::SEV_INFO,
	                          "Purging done: removed=%zu (invalid=%zu, stale=%zu), "
	                          "size_after=%zu, dead_before_purge=%zu, alive_before_purge=%zu",
	                          to_delete.size(),
	                          n_invalid,
	                          n_stale,
	                          table_size_after,
