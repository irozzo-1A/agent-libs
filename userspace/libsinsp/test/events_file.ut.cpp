...
TEST_F(sinsp_with_test_input, creates_fd_generic) {
	...
	fd = 2;
	add_event_advance_ts(increasing_ts(), 1, PPME_SYSCALL_SIGNALFD4_E, 2, (int64_t)0, (uint32_t)0);
	evt = add_event_advance_ts(increasing_ts(),
	                           1,
	...
	fd = 11;
	add_event_advance_ts(increasing_ts(), 1, PPME_SYSCALL_EPOLL_CREATE_E, 1, (int32_t)0);
	evt = add_event_advance_ts(increasing_ts(), 1, PPME_SYSCALL_EPOLL_CREATE_X, 2, fd, (int32_t)0);
	...
	fd = 12;
	add_event_advance_ts(increasing_ts(), 1, PPME_SYSCALL_EPOLL_CREATE1_E, 1, (uint32_t)0);
	evt = add_event_advance_ts(increasing_ts(),
	                           1,
...
TEST_F(sinsp_with_test_input, umount2) {
	...
	evt = add_event_advance_ts(increasing_ts(), 1, PPME_SYSCALL_UMOUNT2_X, 3, res, name, flags);
...
TEST_F(sinsp_with_test_input, pipe) {
	 ...
	 * most one `fdinfo` associated. So in this case the associated file descriptor is the second
	 * one (`4`). Please note that both file descriptors are added to thread info, but in the
	 * `m_fdinfo` field we find only the second file descriptor.
	 */
...
TEST_F(sinsp_with_test_input, pipe2) {
	...
	/* `pipe2` is particular because it generates 2 file descriptors but a single event can have at
	 * most one `fdinfo` associated. So in this case the associated file descriptor is the second
	 * one (`4`). Please note that both file descriptors are added to thread info, but in the
	 * `m_fdinfo` field we find only the second file descriptor.
	 */
...
TEST_F(sinsp_with_test_input, eventfd2) {
	...
	evt = add_event_advance_ts(increasing_ts(), 1, PPME_SYSCALL_EVENTFD2_X, 3, res, flags, initval);
